...
public class CertMgrApplication extends Application {
	...
	void handleUncaughtException(Thread t, Throwable e, UncaughtExceptionHandler next) {
		LOG.error(e, I18N.bundle(), I18N.MESSAGE_UNEXPECTED_EXCEPTION, e.getLocalizedMessage());
		if (next != null) {
	 ...
	 *
	 * @return The currently running application or null, if no application running.
	 */
	...
	private String processParameters() {
		...
		for (Map.Entry<String, String> parameter : parameters.getNamed().entrySet()) {
			LOG.warning(I18N.bundle(), I18N.MESSAGE_INVALID_PARAMETER, parameter.getKey() + "=" + parameter.getValue());
		}
		...
		for (String parameter : parameters.getUnnamed()) {
			if (PARAMETER_VERBOSE.equals(parameter)) {
				...
				LOG.notice(I18N.bundle(), I18N.MESSAGE_VERBOSE_ENABLED);
			} else if (PARAMETER_DEBUG.equals(parameter)) {
				LogConfig.applyConfig(LogConfig.CONFIG_DEBUG);
				LOG.notice(I18N.bundle(), I18N.MESSAGE_DEBUG_ENABLED);
			} else if (openStore == null && isValidStoreParameter(parameter)) {
				openStore = parameter;
			} else {
				LOG.warning(I18N.bundle(), I18N.MESSAGE_INVALID_PARAMETER, parameter);
			}
		}
		return openStore;
	}
	...
	private void logVMInfo() {
		...
		LOG.notice(I18N.bundle(), I18N.MESSAGE_VM_INFO, vmVersion, vmVendor);
	}
	...
	private void logProviderInfo() {
		try {
			LOG.notice(I18N.bundle(), I18N.MESSAGE_PROVIDER_INFO, CertStore.getProviderInfo());
		} catch (Exception e) {
			LOG.error(e, I18N.bundle(), I18N.MESSAGE_PROVIDER_EXCEPTION, e.getLocalizedMessage());
		}
	}
